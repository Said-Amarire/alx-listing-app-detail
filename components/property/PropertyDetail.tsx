      ...
      <div className="flex-1 space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4 mt-4">
          ...
          {property.images?.map((img, idx) => (
            <img key={idx} src={img} alt={`${property.name} ${idx}`} className="w-full h-48 object-cover rounded-lg" />
          ))}
        </div>
        ...
        <div className="mt-4">
          ...
          <ul className="flex flex-wrap gap-2 mt-2">
            {property.category.map((amenity, index) => (
              <li key={index} className="bg-gray-200 p-2 rounded-md text-sm">{amenity}</li>
            ))}
          </ul>
        </div>
